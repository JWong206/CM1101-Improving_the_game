...
def filter_words(words, skip_words):
    ...
    # Define a new list
    filitered = []
    # Loops through the given parameter list
    for x in words:
        if x not in skip_words: # Checks if the word is in the list of skipped words
            filitered.append(x) # If it is not in the list, adds word to new list
    # Returns the filitered words
    return(filitered)
...
def normalise_input(user_input):
    ...
    # Splits each word up and put into a list
    split_words = no_punct.split()
    # Loops through the list and removes start/end spaces
    for x in split_words:
        x.strip()
    # Filiters out unwanted words
    normalised = filter_words(split_words, skip_words)
    # Returns normalised words
    return(normalised)
